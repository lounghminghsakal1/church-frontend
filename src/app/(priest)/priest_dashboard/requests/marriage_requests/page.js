...
// ─── main page ────────────────────────────────────────────────────────────────
const ViewAllMarriageRequestsPage = () => {
  const router = useRouter();
  const [requests, setRequests] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [submitting, setSubmitting] = useState({}); // { [id]: "approved" | "rejected" | null }
  const [openFamilyCardViewModal, setOpenFamilyCardViewModal] = useState(false);

  const fetchRequests = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await apiGet("/priest/marriage_requests");
  ...
  const handleReview = async (req, decision) => {
    ...
    try {
      const res = await apiPost(`/priest/marriage_requests/${req._id}`, {
        status: decision,
      ...
      // optimistic UI — mark the row with its new status
      setRequests((prev) =>
        prev.map((r) =>
          r._id === req._id ? { ...r, status: decision } : r
        )
      );
    } catch (err) {
      toast.error(err.message);
  ...
  if (error) return <PageShell><ErrorState message={error} onRetry={fetchRequests} /></PageShell>;
  if (requests.length === 0) return <PageShell><EmptyState /></PageShell>;
      ...
      <div className="grid grid-cols-1 gap-5">
        {requests.map((req) => (
          <MarriageRequestCard
            ...
            openFamilyCardViewModal={openFamilyCardViewModal}
            setOpenFamilyCardViewModal={setOpenFamilyCardViewModal}
          />
        ))}
      </div>
    </PageShell>
...
// ─── request card ─────────────────────────────────────────────────────────────
function MarriageRequestCard({ req, submitting, onResponseChange, onApprove, onReject, openFamilyCardViewModal, setOpenFamilyCardViewModal }) {
  const isActedOn = req.status && req.status !== "pending";
  const isLoading = !!submitting;
  const fileUrl =
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/${req.user.family_card_document.file_path
      .replaceAll("\\", "/")
    }`;
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#0F2A4A] px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-rose-100 flex items-center justify-center">
            <Heart className="w-4 h-4 text-rose-500" strokeWidth={1.5} />
          </div>
          <span className="text-white text-sm font-medium">Marriage Request</span>
        </div>
        {req.status && (
      ...
      <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <InfoRow icon={<Dock className="w-4 h-4" />} label="Family Card Document">
          <button
            onClick={() => setOpenFamilyCardViewModal(true)}
            className="inline-flex items-center gap-2 bg-[#0F2A4A] hover:bg-[#1a3d6b] text-white text-xs font-semibold px-3.5 py-2 rounded-lg transition-colors group"
          >
            <FileText className="w-3.5 h-3.5 text-[#C9A84C] flex-shrink-0" strokeWidth={1.5} />
            <span>View Family Card</span>
            <ExternalLink className="w-3 h-3 text-white/30 group-hover:text-[#C9A84C] transition-colors flex-shrink-0" />
          </button>
        </InfoRow>

        <InfoRow icon={<User className="w-4 h-4" />} label="Groom">
          <span>{req.groom_name ?? "—"}</span>
          {req.groom_location && (
            <span className="flex items-center gap-1 text-gray-400 text-xs mt-0.5">
              <MapPin className="w-3 h-3" /> {req.groom_location}
            </span>
          )}
          {req.groom_mobile_number && (
            <span className="flex items-center gap-1 text-gray-400 text-xs mt-0.5">
              <Phone className="w-3 h-3" /> {req.groom_mobile_number}
            </span>
          )}
        </InfoRow>

        <InfoRow icon={<User className="w-4 h-4" />} label="Bride">
          <span>{req.bride_name ?? "—"}</span>
          {req.bride_location && (
            <span className="flex items-center gap-1 text-gray-400 text-xs mt-0.5">
              <MapPin className="w-3 h-3" /> {req.bride_location}
            </span>
          )}
          {req.bride_mobile_number && (
            <span className="flex items-center gap-1 text-gray-400 text-xs mt-0.5">
              <Phone className="w-3 h-3" /> {req.bride_mobile_number}
            </span>
          )}
        </InfoRow>

        <InfoRow icon={<CalendarDays className="w-4 h-4" />} label="Preferred Marriage date">
          <span>{safeFormat(req.preferred_marriage_date_and_time)}</span>
        </InfoRow>
      </div>

      {/* Response + actions — hidden if already acted on */}
      {!isActedOn && (
        <div className="px-5 pb-5 border-t border-[#0F2A4A]/5 pt-4">
          ...
          <textarea
            rows={2}
            placeholder="Enter a message for the family — e.g. confirmed date, instructions, or reason for rejection…"
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-gray-400">All Marriage requests have been reviewed.</p>
    </div>
  );
}

export default ViewAllMarriageRequestsPage;