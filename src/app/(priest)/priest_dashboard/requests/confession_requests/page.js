...
// ─── main page ────────────────────────────────────────────────────────────────
const ViewAllConfessionRequestsPage = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiGet("/priest/confession_requests");
  ...
  const handleReview = async (req, decision) => {
    ...
    try {
      const res = await apiPost(`/priest/confession_requests/${req._id}`, {
        status: decision,
...
// ─── request card ─────────────────────────────────────────────────────────────
function ConfessionRequestCard({ req, submitting, onResponseChange, onApprove, onReject }) {
    ...
    >
      ...
      <div className="bg-[#0F2A4A] px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white text-sm font-medium">Confession Request</span>
        </div>
        {req.status && (
      ...
      <div className="p-5 grid grid-cols-1 sm:grid-cols-2  gap-4">
        ...
        <InfoRow icon={<PersonStandingIcon className="w-4 h-4" />} label="Person">
          <span>{req.confession_person_name ?? "—"}</span>
        </InfoRow>

        <InfoRow icon={<CalendarDays className="w-4 h-4" />} label="Preferred Confession date">
          <span>{safeFormat(req.preferred_confession_date_and_time)}</span>
        </InfoRow>
      </div>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-gray-400">All Confession requests have been reviewed.</p>
    </div>
  );
}

export default ViewAllConfessionRequestsPage;