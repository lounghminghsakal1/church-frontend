...
// ─── main page ────────────────────────────────────────────────────────────────
const ViewAllConfirmationRequestsPage = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiGet("/priest/confirmation_requests");
  ...
  const handleReview = async (req, decision) => {
    ...
    try {
      const res = await apiPost(`/priest/confirmation_requests/${req._id}`, {
        status: decision,
...
function PageShell({ children, loading, onRefresh }) {
  const router = useRouter();
  return (
...
// ─── request card ─────────────────────────────────────────────────────────────
function ConfirmationRequestCard({ req, submitting, onResponseChange, onApprove, onReject, openFamilyCardViewModal, setOpenFamilyCardViewModal }) {
    ...
    >
      ...
      <div className="bg-[#0F2A4A] px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white text-sm font-medium">Confirmation Request</span>
        </div>
        {req.status && (
      ...
      <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <InfoRow icon={<PersonStanding className="w-4 h-4" />} label="Confirmation Person">
          <span>{req.confirmation_person_name ?? "—"}</span>
        </InfoRow>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-gray-400">All Confirmation requests have been reviewed.</p>
    </div>
  );
}

export default ViewAllConfirmationRequestsPage;