...
// ─── main page ────────────────────────────────────────────────────────────────
const ViewAllEucharistRequestsPage = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiGet("/priest/eucharist_requests");
  ...
  const handleReview = async (req, decision) => {
    ...
    try {
      const res = await apiPost(`/priest/eucharist_requests/${req._id}`, {
        status: decision,
...
// ─── request card ─────────────────────────────────────────────────────────────
function EucharistRequestCard({ req, submitting, onResponseChange, onApprove, onReject, openFamilyCardViewModal, setOpenFamilyCardViewModal }) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#0F2A4A] px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-sky-100 flex items-center justify-center">
            <Droplets className="w-4 h-4 text-sky-500" strokeWidth={1.5} />
          </div>
          <span className="text-white text-sm font-medium">Eucharist Request</span>
        </div>
        {req.status && (
      ...
      <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <InfoRow icon={<User className="w-4 h-4" />} label="Parents">
          <span>{[req.father_name, req.mother_name].filter(Boolean).join(" & ") || "—"}</span>
        </InfoRow>

        <InfoRow icon={<PersonStanding className="w-4 h-4" />} label="Eucharist Person">
          <span>{req.eucharist_person_name ?? "—"}</span>
        </InfoRow>

        <InfoRow icon={<CalendarDays className="w-4 h-4" />} label="Preferred Eucharist date">
          <span>{safeFormat(req.preferred_eucharist_date_and_time)}</span>
        </InfoRow>
      </div>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-gray-400">All Eucharist requests have been reviewed.</p>
    </div>
  );
}

export default ViewAllEucharistRequestsPage;